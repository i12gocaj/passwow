...
def test_export_success(tmp_path):
    ...
    runner.invoke(cli, ['init', '--path', str(vault_file)], input=f"{master_pw}\n{master_pw}\n")
    runner.invoke(
        cli,
...
def test_import_success(tmp_path):
    ...
    runner.invoke(cli, ['init', '--path', str(vault_src)], input=f"{master_pw}\n{master_pw}\n")
    runner.invoke(
        cli,
    ...
    result = runner.invoke(
        cli,
        ['import', '--path', str(vault_dest), '--file', str(vault_src)]
    )
    assert result.exit_code == 0
    assert f"Vault importado desde '{vault_src}' a '{vault_dest}' correctamente." in result.output
...
def test_import_no_source(tmp_path):
    ...
    result = runner.invoke(
        cli,
        ['import', '--path', str(vault_dest), '--file', str(src)]
    )
    assert result.exit_code == 0
