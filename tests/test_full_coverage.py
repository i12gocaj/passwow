...
from vault.cli import cli, _handle_load_entries
...
def test_add_generate_no_pyperclip(monkeypatch, tmp_path):
    ...
    result = runner.invoke(
        cli,
        ["add", "--path", str(vault_file), "--name", "g", "--user", "u", "--generate"],
        input=f"{pw}\n10\n",
    )
...
def test_delete_cancel(tmp_path):
    ...
    pw = "pw1"
    runner.invoke(cli, ["init", "--path", str(vault_file)], input=f"{pw}\n{pw}\n")
    # Enviar contraseña y luego "no" para cancelar
    result = runner.invoke(
...
def test_no_checksum(tmp_path):
    ...
    if checksum.exists():
        checksum.unlink()
    result = runner.invoke(
        cli, ["get", "--path", str(vault_file), "--name", "x"], input=f"{pw}\n"
    )
    assert result.exit_code == 0
...
def test_export_import_error(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        shutil, "copy2", lambda *a, **kw: (_ for _ in ()).throw(OSError("fail"))
    )
    result = runner.invoke(
        cli,
        ["export", "--path", str(vault_file), "--file", str(tmp_path / "x.dat")],
        input=f"{pw}\n",
    )
    ...
    result = runner.invoke(
        cli, ["import", "--path", str(vault_file), "--file", str(src)]
    )
...
def test_changepw_wrong_password(tmp_path):
    ...
    runner.invoke(cli, ["init", "--path", str(vault_file)], input=f"{pw}\n{pw}\n")
    result = runner.invoke(
        cli, ["changepw", "--path", str(vault_file)], input="wrong\nnew\nnew\n"
    )
...
def test_pwned_network_error(monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr(
        "requests.get", lambda *a, **k: (_ for _ in ()).throw(Exception("fail"))
    )
...
def test_session_edge_cases(tmp_path, monkeypatch):
    ...
    session_file.write_text(
        json.dumps(
            {"key": "aGVsbG8=", "timestamp": int(time.time()), "checksum": "bad"}
        )
    )
    assert vault.session.load_session(str(session_file)) is None
    # Expirado
    ...
    data = {
        "key": "aGVsbG8=",
        "timestamp": ts,
        "checksum": vault.session._calculate_checksum(
            json.dumps({"key": "aGVsbG8=", "timestamp": ts}, sort_keys=True)
        ),
    }
...
def test_add_entries_none(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        cli,
        ["add", "--path", str(tmp_path / "vault.dat"), "--name", "n", "--user", "u"],
        input="pw\n",
    )
    assert result.exit_code == 0
...
def test_get_entries_none(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vault.cli._handle_load_entries", lambda *a, **k: None)
    result = runner.invoke(
        cli, ["get", "--path", str(tmp_path / "vault.dat"), "--name", "n"], input="pw\n"
    )
    assert result.exit_code == 0
...
def test_get_entry_not_found(tmp_path):
    ...
    runner.invoke(cli, ["init", "--path", str(vault_file)], input=f"{pw}\n{pw}\n")
    result = runner.invoke(
        cli, ["get", "--path", str(vault_file), "--name", "nope"], input=f"{pw}\n"
    )
...
def test_list_entries_none_and_empty(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vault.cli._handle_load_entries", lambda *a, **k: None)
    result = runner.invoke(
        cli, ["list", "--path", str(tmp_path / "vault.dat")], input="pw\n"
    )
    assert result.exit_code == 0
    # Ahora lista vacía
    vault_file = tmp_path / "vault.dat"
...
def test_remove_entries_none_and_not_found(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        cli,
        ["remove", "--path", str(tmp_path / "vault.dat"), "--name", "n"],
        input="pw\n",
    )
    assert result.exit_code == 0
    ...
    runner.invoke(cli, ["init", "--path", str(vault_file)], input=f"{pw}\n{pw}\n")
    result = runner.invoke(
        cli, ["remove", "--path", str(vault_file), "--name", "nope"], input=f"{pw}\n"
    )
    out = result.output.lower()
...
def test_changepw_vault_not_exists_and_entries_none(tmp_path, monkeypatch):
    runner = CliRunner()
    result = runner.invoke(
        cli, ["changepw", "--path", str(tmp_path / "vault.dat")], input="pw\n"
    )
...
def test_export_branches(tmp_path, monkeypatch):
    ...
    # Vault no existe
    result = runner.invoke(
        cli, ["export", "--path", str(vault_file), "--file", str(dest)], input=f"{pw}\n"
    )
    assert "no se encontró" in result.output.lower()
    # Vault existe pero checksum corrupto
    vault_file.write_bytes(b"abc")
    ...
    monkeypatch.setattr(
        "vault.storage.load_entries",
        lambda *a, **k: (_ for _ in ()).throw(InvalidTag()),
    )
    result = runner.invoke(
        cli, ["export", "--path", str(vault_file), "--file", str(dest)], input=f"{pw}\n"
    )
    ...
    assert (
        "incorrecta" in out
        or "no se pudo" in out
        or "manipulado" in out
        or "[alerta]" in out
    )
    # Exception
    monkeypatch.setattr(
    ...
    result = runner.invoke(
        cli,
        ["export", "--path", str(vault_file), "--file", str(dest), "--format", "json"],
        input=f"{pw}\n",
    )
    assert "json" in result.output.lower() or result.exit_code == 0
    result = runner.invoke(
        cli,
        ["export", "--path", str(vault_file), "--file", str(dest), "--format", "csv"],
        input=f"{pw}\n",
    )
...
def test_import_vault_branches_cli(tmp_path, monkeypatch):
    ...
    # Excepción
    monkeypatch.setattr(
        "shutil.copy2", lambda *a, **k: (_ for _ in ()).throw(Exception("fail"))
    )
    result = runner.invoke(cli, ["import", "--path", str(dest), "--file", str(src)])
