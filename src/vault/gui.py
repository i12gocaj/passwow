...
class PasswowGUI:
    def __init__(self, root):
        ...
        # Usar el directorio del ejecutable o script como base para vault.dat
        if getattr(sys, "frozen", False):
            base_dir = os.path.dirname(sys.executable)
        else:
            base_dir = os.path.dirname(os.path.abspath(__file__))
        self.vault_path = os.path.join(base_dir, "vault.dat")
    ...
    def create_main_menu(self):
        ...
        vault_menu.add_command(
            label="Cambiar Contraseña Maestra", command=self.change_master_password
        )
        ...
        tools_menu.add_command(
            label="Comprobar Contraseña (Pwned)", command=self.check_pwned
        )
    ...
    def show_welcome(self):
        ...
        label = tk.Label(
            self.root,
            text="Bienvenido a Passwow\nGestor de contraseñas local y seguro",
            font=("Arial", 16),
            pady=20,
        )
        label.pack()
    ...
    def init_vault(self):
        ...
        password = simpledialog.askstring(
            "Contraseña Maestra", "Introduce una nueva contraseña maestra:", show="*"
        )
        if not password:
            return
        if Path(self.vault_path).exists():
            messagebox.showwarning(
                "Vault existente", f"El vault '{self.vault_path}' ya existe."
            )
            return
        ...
        # Crear el archivo de checksum igual que la CLI
        from vault.cli import _write_vault_checksum

        _write_vault_checksum(self.vault_path)
        messagebox.showinfo(
            "Vault creado", f"Vault inicializado correctamente en '{self.vault_path}'."
        )
    ...
    def change_master_password(self):
        ...
        if not Path(self.vault_path).exists():
            messagebox.showerror(
                "Error", f"No se encontró el vault '{self.vault_path}'."
            )
            return
        old_pw = simpledialog.askstring(
            "Contraseña actual", "Introduce la contraseña maestra actual:", show="*"
        )
        if not old_pw:
            return
        try:
        ...
        new_pw = simpledialog.askstring(
            "Nueva contraseña", "Introduce la nueva contraseña maestra:", show="*"
        )
        if not new_pw:
            return
        try:
    ...
    def export_vault(self):
        ...
        password = simpledialog.askstring(
            "Contraseña Maestra", "Introduce la contraseña maestra:", show="*"
        )
        if not password:
            return
        if not Path(self.vault_path).exists():
        ...
        export_type = simpledialog.askstring(
            "Formato de exportación", "Formato (bin/json/csv):", initialvalue="bin"
        )
        if not export_type or export_type not in ("bin", "json", "csv"):
            messagebox.showwarning(
                "Formato inválido", "El formato debe ser bin, json o csv."
            )
            return
        dest_path = filedialog.asksaveasfilename(
            title="Guardar como", defaultextension=export_type
        )
        if not dest_path:
            return
        try:
            if export_type == "bin":
                ...
                Path(dest_path).chmod(0o600)
            messagebox.showinfo(
                "Exportado", f"Vault exportado correctamente a '{dest_path}'."
            )
        except Exception as e:
    ...
    def import_vault(self):
        ...
        if Path(self.vault_path).exists():
            overwrite = messagebox.askyesno(
                "Sobrescribir",
                f"El vault '{self.vault_path}' ya existe. ¿Sobrescribir?",
            )
            if not overwrite:
                return
        try:
            ...
            Path(self.vault_path).chmod(0o600)
            messagebox.showinfo(
                "Importado", f"Vault importado correctamente desde '{src_path}'."
            )
        except Exception as e:
    ...
    def delete_vault(self):
        ...
        if not Path(self.vault_path).exists():
            ...
            return
        # Solicitar contraseña maestra antes de permitir el borrado
        password = simpledialog.askstring(
            "Contraseña Maestra",
            "Introduce la contraseña maestra para borrar el vault:",
            show="*",
        )
        if not password:
            return
        try:
            # Intentar descifrar el vault para validar la contraseña
            load_entries(self.vault_path, password)
        except Exception:
            messagebox.showerror(
        ...
        confirm = simpledialog.askstring(
            "Confirmar borrado",
            "Escribe BORRAR para eliminar el vault y todos sus datos:",
        )
        if confirm != "BORRAR":
        ...
        try:
            ...
            messagebox.showinfo(
                "Eliminado", "Vault y datos eliminados de forma segura."
            )
        except Exception as e:
    ...
    def add_entry(self):
        ...
        gen = messagebox.askyesno(
            "Contraseña", "¿Generar contraseña segura automáticamente?"
        )
        if gen:
            ...
            entry_pass = simpledialog.askstring(
                "Contraseña", "Contraseña para la entrada:", show="*"
            )
            if not entry_pass:
                return
        new_entry = {
    ...
    def get_entry(self):
        ...
        name = simpledialog.askstring(
            "Buscar Entrada", "Nombre de la entrada a recuperar:"
        )
        if not name:
            return
        ...
        if not entry:
            messagebox.showwarning(
                "No encontrada", f"No existe una entrada con nombre '{name}'."
            )
            return
    ...
    def remove_entry(self):
        ...
        try:
            ...
            return
        name = simpledialog.askstring(
            "Eliminar Entrada", "Nombre de la entrada a eliminar:"
        )
        if not name:
            return
        ...
        try:
            ...
            messagebox.showinfo(
                "Eliminada", f"Entrada '{name}' eliminada correctamente."
            )
        except Exception as e:
    ...
    def check_pwned(self):
        ...
        pw = simpledialog.askstring(
            "Comprobar contraseña", "Introduce la contraseña a comprobar:", show="*"
        )
        if not pw:
            return
        sha1 = hashlib.sha1(pw.encode()).hexdigest().upper()  # nosec
        ...
        try:
            ...
            if resp.status_code != 200:
                messagebox.showerror(
                    "Error", "No se pudo consultar la API de HaveIBeenPwned."
                )
                return
            ...
            for suf, count in hashes:
                if suf == suffix:
                    messagebox.showwarning(
                        "¡Comprometida!",
                        f"Esta contraseña ha aparecido {count} veces en filtraciones.",
                    )
                    return
            messagebox.showinfo(
                "OK",
